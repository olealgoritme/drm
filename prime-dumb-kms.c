// This will works on Embedded GPU that implements .gem_prime_mmap like Rockchip
// ones. This will fail on most DRM drivers for GPU with dedicated memory as
// they tend to NOT implement .gem_prime_mmap.
...
int main() {
  ...
  if (!chosen_resolution) {
    LOG("No preferred resolution on the selected connector %u ?\n",
   ...
   * out quickly. */
  if (!screen_encoder) {
    LOG("Could not retrieve the encoder for mode %s on connector %u.\n",
        chosen_resolution->name, valid_connector->connector_id);
    ret = -ENOLINK;
  ...
  ret = drmModeAddFB(drm_fd, chosen_resolution->hdisplay,
                     chosen_resolution->vdisplay, 24, 32, create_request.pitch,
  ...
  ret = drmModeSetCrtc(drm_fd, current_crtc_id, frame_buffer_id, 0, 0,
                       &valid_connector->connector_id, 1, chosen_resolution);
  ...
  struct drm_prime_handle prime_request = {.handle = create_request.handle,
   ...
   * purpose of our test */
  if (ret || dma_buf_fd < 0) {
    LOG("Could not export buffer : %s (%d) - FD : %d\n", strerror(ret), ret,
        dma_buf_fd);
  ...
  uint8_t *primed_framebuffer =
      mmap(0, create_request.size, PROT_READ | PROT_WRITE, MAP_SHARED,
           dma_buf_fd, 0);
  ret = errno;

  /* Bail out if we could not map the framebuffer using this method */
  if (primed_framebuffer == NULL || primed_framebuffer == MAP_FAILED) {
    LOG("Could not map buffer exported through PRIME : %s (%d)\n"
        "Buffer : %p\n",
        strerror(ret), ret, primed_framebuffer);
  ...
  uint_fast64_t pixel = 0, size = create_request.size;
...
could_not_export_buffer:
  ...
  {
    struct drm_mode_destroy_dumb destroy_request = {.handle =
                                                        create_request.handle};

    ioctl(drm_fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy_request);
  }
  drmModeSetCrtc(drm_fd, crtc_to_restore->crtc_id, crtc_to_restore->buffer_id,
                 0, 0, &valid_connector->connector_id, 1,
