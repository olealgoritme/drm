...
void runEGL(const DmaBuf *img) {
  ...
  EGLImage eimg = eglCreateImage(edisp, EGL_NO_CONTEXT, EGL_LINUX_DMA_BUF_EXT,
                                 (EGLClientBuffer)NULL, eimg_attrs);
  ASSERT(eimg);
...
int main(int argc, const char *argv[]) {
  ...
  MSG("Received width=%d height=%d pitch=%u fourcc=%#x fd=%d", img.width,
      img.height, img.pitch, img.fourcc, img.fd);
...
cleanup:
  ...
  MSG("fb_id=%#x width=%u height=%u pitch=%u bpp=%u depth=%u handle=%#x", fb_id,
