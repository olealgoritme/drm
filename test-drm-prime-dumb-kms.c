...
int main()
{
	...
	int const kms_fd = open("/dev/dri/card0", O_RDWR | O_CLOEXEC);

	if (kms_fd < 0) {
		LOG("Could not open /dev/dri/card0 : %m\n");
		goto could_not_open_kms_fd;
	}
	...
	for (int_fast32_t c = 0; c < drm_resources->count_connectors; c++) {
		valid_connector =
			drmModeGetConnector(kms_fd, drm_resources->connectors[c]);
	...
	screen_encoder =
		drmModeGetEncoder(kms_fd, valid_connector->encoder_id);
	...
	ret = ioctl(kms_fd, DRM_IOCTL_MODE_CREATE_DUMB, &create_request);
	...
	drmModeCrtc * __restrict crtc_to_restore =
		drmModeGetCrtc(kms_fd, current_crtc_id);
	...
	ret = drmModeSetCrtc(
		kms_fd, current_crtc_id, frame_buffer_id,
		0, 0,
		&valid_connector->connector_id,
		1,
		chosen_resolution);
	...
	ret = ioctl(kms_fd, DRM_IOCTL_PRIME_HANDLE_TO_FD, &prime_request);
	...
	uint32_t const colors[] = {red, green, blue, white};
	...
	while (pixel < size_in_pixels) {
		...
		uint32_t current_color = colors[rand()%ARRAY_SIZE(colors)];
		...
		for (uint_fast32_t p = 0; p < width_pixel; p++) {
			if (p > 700) current_color ^= 0xffffff;
			((uint32_t *) primed_framebuffer)[pixel++] = current_color;
		}
		pixel += diff_between_width_and_stride;
		LOG("pixel : %lu, size : %lu, width_pixel : %u\n", pixel, size_in_pixels, width_pixel);
		usleep(100000);
	}
...
could_not_export_buffer:
	// very ugly but will do for an example...
	{
		...
		ioctl(kms_fd, DRM_IOCTL_MODE_DESTROY_DUMB, &destroy_request);
	}
	drmModeSetCrtc(
		kms_fd,
...
could_not_retrieve_current_crtc:
	drmModeRmFB(kms_fd, frame_buffer_id);
